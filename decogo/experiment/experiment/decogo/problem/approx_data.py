...
class InnerPoints:
    ...
    def get_min_column(self, block_id, dir_im_space):
        ...
        in image space, i.e. :math:`points = {\\mathrm{argmin\ }} d^Tx, x \\in S`,
...
class LinearizationCuts:
    ...
    def add_linearization_cuts(self, y, eps, x=None, block_id=None):
        ...
        The cuts are computed with formula :math:`g(points) + \\nabla g(points)^T(x-points)
        \\leq 0`, where :math:`points` is a linearization point.
        ...
        * if ``x`` is not ``None``, then the cuts are added only for \
        constraint which are violated at point x and are  active at point points
        ...
        * if ``block_id`` is ``None``, then linearization cuts are added to \
        ...
        :rtype: int
        """
        # linearization cuts at point points are the following: g(points) +
        # grad g(points)*(x-points) <= 0
    ...
    def compute_and_add_linearization_cut(self, block_id, j, y):
        ...
        of the k-th block regarding a reference point points
        ...
        # local linear constraint creation g(points) + grad g(points)*(x-points) <= 0
        # reformulated grad g(points)*x <= grad(points)*points - g(points), points is given point,
        # x is variable
        lhs = []
        rhs = 0
        ...
        # eval the function g at point points
