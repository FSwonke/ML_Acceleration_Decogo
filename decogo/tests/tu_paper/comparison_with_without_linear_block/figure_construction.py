...
def parse_files_construct_table(instances, logs_path_name='tests/solver/colgen'):
    ...
    start = get_rounded(start)
    ax.yaxis.set_ticks(np.arange(start, end, 1e7))

    start, end = ax.get_xlim()
    ax.xaxis.set_ticks(np.arange(0, end, 4e4))

    ax.legend()
    plt.tight_layout()
    fig.savefig('cg_relaxation_with_without_linear_block_integration.eps')


if __name__ == '__main__':

    instances = ['without linear block integration',
                 'with linear block integration']
    instances.sort()
    logs_path_name = 'tests/stats/tu_paper/comparison_with_without_linear_block'
